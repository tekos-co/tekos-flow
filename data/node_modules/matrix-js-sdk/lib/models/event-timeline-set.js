...
EventTimelineSet.prototype.addEventsToTimeline = function (events, toStartOfTimeline, timeline, paginationToken) {
    ...
    for (var i = 0; i < events.length; i++) {
        ...
        if (backwardsIsLive || forwardsIsLive) {
            ...
            console.warn({ backwardsIsLive: backwardsIsLive, forwardsIsLive: forwardsIsLive }); // debugging
            if (backwardsIsLive) {
                console.warn("Refusing to set a preceding existingTimeLine on our " + "timeline as the existingTimeLine is live (" + existingTimeline + ")");
            }
            if (forwardsIsLive) {
                console.warn("Refusing to set our preceding timeline on a existingTimeLine " + "as our timeline is live (" + timeline + ")");
            }
    ...
    if (lastEventWasNew || !didUpdate) {
        if (direction === EventTimeline.FORWARDS && timeline === this._liveTimeline) {
            console.warn({ lastEventWasNew: lastEventWasNew, didUpdate: didUpdate }); // for debugging
            console.warn("Refusing to set forwards pagination token of live timeline " + (timeline + " to " + paginationToken));
            return;
        }
