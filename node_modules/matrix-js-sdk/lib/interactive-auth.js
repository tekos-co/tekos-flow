...
InteractiveAuth.prototype = {
     ...
     */
    _doRequest: function () {
        var _ref = (0, _bluebird.coroutine)( /*#__PURE__*/_regenerator2.default.mark(function _callee(auth, background) {
                ...
                while (1) {
                    switch (_context.prev = _context.next) {
                        ...
                        case 7:
                            ...
                            errorFlows = _context.t0.data ? _context.t0.data.flows : null;
                            haveFlows = Boolean(this._data.flows) || Boolean(errorFlows);

                            if (_context.t0.httpStatus !== 401 || !_context.t0.data || !haveFlows) {
                                // doesn't look like an interactive-auth failure.
                                if (!background) {
                                    this._rejectFunc(_context.t0);
                                } else {
                                    // We ignore all failures here (even non-UI auth related ones)
                                    // since we don't want to suddenly fail if the internet connection
                                    // had a blip whilst we were polling
                                    console.log("Background poll request failed doing UI auth: ignoring", _context.t0);
                                }
                            }
                            ...
                            // we workaround here.
                            if (!_context.t0.data.flows && !_context.t0.data.completed && !_context.t0.data.session) {
                                _context.t0.data.flows = this._data.flows;
                                _context.t0.data.completed = this._data.completed;
                                _context.t0.data.session = this._data.session;
                            }
                            this._data = _context.t0.data;
                            this._startNextAuthStage();

                            if (!(!this._emailSid && this._chosenFlow.stages.includes('m.login.email.identity'))) {
                                _context.next = 26;
                                break;
                            }
                        ...
                        case 19:
                            requestTokenResult = _context.sent;
    ...
    _chooseStage: function _chooseStage() {
        ...
        console.log("Active flow => %s", (0, _stringify2.default)(this._chosenFlow));
