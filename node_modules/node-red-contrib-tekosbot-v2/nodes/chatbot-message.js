  ...
  function ChatBotMessage(config) {
    ...
    this.on('input', function(msg) {
      ...
      for (var i = 0; i < this.message.length; i++) {
          if (this.message[i].message.includes("{{")){
            
            listMessages[i] = mustache.render(this.message[i].message,msg.payload)
          }else{
            listMessages[i] = this.message[i].message;
          }
      ...
      } else if (_.isArray(listMessages) && !this.emptyMessages(listMessages)) {
        message = node.pickOne(listMessages);
