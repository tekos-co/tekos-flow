<script type="text/javascript">
  ...
  $.RedBot.validate.src = function(src) {
    return src != null && src.match(/[-a-zA-Z0-9@:%_\+.~#?&//=]{2,256}\.[a-z]{2,4}\b(\/[-a-zA-Z0-9@:%_\+.~#?&//=]*)?/gi);
  };
  ...
  $.RedBot.validate.button = function(button) {
    ...
    switch(button.type) {
      ...
      case 'image':
        return $.RedBot.validate.notEmpty(button.label) && $.RedBot.validate.notEmpty(button.src);
      case 'video':
        return $.RedBot.validate.notEmpty(button.label) && $.RedBot.validate.notEmpty(button.src);
      case 'postback':
  ...
  $.fn.RB_setMediaData = function(obj, options) {
    options = options || {};
    options.badges = options.badges !== false;
    var container = $(this);
    ...
    $('input[name=src]', panel).val(obj.src);
    $('input[name=value]', panel).val(obj.value);
  ...
  $.fn.RB_getMediaData = function(type) {
    var container = $(this);
    let panel = container.find('.panel-'+type)
    let label = panel.find('input[name=label]')
    let src = panel.find('input[name=src]') 
    var result = null;
    switch (type){
      case 'image':
        result = {
  ...
  $.fn.RB_mountMediaDialog = function(options){
    options = options || {};
    ...
    var panelImage = $('<div class="panel panel-image" style="display:none;"/>').appendTo(container);
    $('<div/>').css('marginTop','5px')
      .append($('<input style="width:100%" class="node-input-rule-type" type="text" name="label" placeholder="Label"/>'))
      .appendTo(panelImage);
    $('<div/>').css('marginTop','5px')
      .append($('<input style="width:100%" class="node-input-rule-type" type="text" name="src" placeholder="Src"/>'))
      .appendTo(panelImage);


    var panelVideo = $('<div class="panel panel-video" style="display:none;"/>').appendTo(container);
    $('<div/>').css('marginTop','5px')
      .append($('<input style="width:100%" class="node-input-rule-type" type="text" name="label" placeholder="Label"/>'))
      .appendTo(panelVideo);
    $('<div/>').css('marginTop','5px')
  ...
  $.fn.RB_mountButtonDialog = function(options) {
    ...
    options.types = options.types != null ?
      options.types : ['url', 'postback', 'call', 'share', 'login', 'newline', 'logout', 'location', 'quick-reply', 'dialog', 'key', 'image'];
    options.platforms = options.platforms != null ? options.platforms : ['facebook', 'telegram'];
    options.badges = options.badges !== false;
    var container = $(this);
    var main = $('<div/>').appendTo(container);
    var cx = options.badges ? $.RedBot.platformClass : function() { return ''; };
    // if(options.types[0] !== 'image'){
    ...
    if(options.types[0] !== 'image')
        ...
        selectButtonType.append($('<option value="">Select type</option>'));
      ...
      //     // store class only if badges should be shown
      //     if (button != null && options.badges) {
      //       $(self).attr('class', button.className);
      //     } else {
      //       $(self).attr('class', '');
      //     }
    ...
    $('<div/>').css('marginTop','5px')
      ...
      .appendTo(panelUrl);
    $('<div/>').css('marginTop','5px')
      .append($('<input style="width:100%" class="node-input-rule-type" type="text" name="url" placeholder="URL"/>'))
      .appendTo(panelUrl);
    // $('<div/>').css('marginTop','5px')
    ...
    $('<div/>').css('marginTop','5px')
      .append($('<input style="width:100%" class="node-input-rule-type" type="text" name="value" placeholder="Button value">'))
      .appendTo(panelQuickReply);
    // $('<div/>').css('marginTop','5px')
